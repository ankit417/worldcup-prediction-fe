...
import { GAME, ADD_GAME, UPDATE_GAME, DELETE_GAME } from '.'
...
export function deleteGameAction(id: number, callBack?: any) {
  return async function (dispatch: any) {
    let res
    try {
      dispatch({ type: DELETE_GAME.LOADING })
      res = await api(`${APIS.game}/${id}`, 'DELETE')
      //console.log('team', res)
      const { success, message } = res.data
      //console.log('res data add group', message)
      if (success) {
        dispatch({
          type: DELETE_GAME.SUCCESS,
        })
        toast.success('Game Deleted')
        callBack && callBack()
        return 1
      } else {
        dispatch({ type: DELETE_GAME.ERROR })
        toast.error(message)
        return 0
      }
    } catch ({ message }) {
